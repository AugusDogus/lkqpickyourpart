...
export function SearchResults({ searchResult, isLoading }: SearchResultsProps) {
  ...
  const vehicles = useMemo(() => deferredResult?.vehicles ?? [], [deferredResult]);
  ...
  }, [isLoading, visibleCount, vehicles.length]);

  return (
    <div className="space-y-6">
      {/* Results Grid or Loading Skeletons */}
      {isLoading ? (
      ...
      <div className="text-muted-foreground border-t pt-6 text-center text-sm">
        <p>
          Showing {isLoading ? 0 : visibleVehicles.length} of {deferredResult.totalCount.toLocaleString()} vehicles
